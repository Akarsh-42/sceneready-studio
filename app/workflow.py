...
async def run_workflow(brief, provider, demo=False):
    run_id, started = uuid4().hex[:12], time.monotonic()
    warnings, trace = [], []
    if demo:
    ...
    trace.append({"stage": "breakdown", "seconds": round(time.monotonic() - stage_start, 2)})
    ...
    trace.append({"stage": "research", "seconds": round(time.monotonic() - stage_start, 2), "searches": searches})
    ...
    trace.append({"stage": "planning", "seconds": round(time.monotonic() - stage_start, 2)})
    ...
    trace.append({"stage": "validation", "seconds": round(time.monotonic() - stage_start, 3)})
